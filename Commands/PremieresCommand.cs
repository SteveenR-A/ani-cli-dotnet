...
{
    public class PremieresCommand
    {
        private readonly AppState _state;
        private readonly PlaybackController _playback;

        public PremieresCommand(AppState state, PlaybackController playback)
        {
            _state = state;
            _playback = playback;
        }
        ...
        public async Task ExecuteAsync()
        {
            ...
            await AnsiConsole.Status()
                ...
                {
                    results = await DataCache.GetOrFetchDataAsync($"premieres_{_state.ActiveExtractor.Domain}", TimeSpan.FromMinutes(5), () => _state.ActiveExtractor.GetPremieresAsync());
                });
